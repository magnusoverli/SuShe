...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QTextBrowser, QMessageBox, QFileDialog, QComboBox, QDoubleSpinBox
)
from PyQt6.QtCore import Qt, pyqtSignal, QLocale
import logging
import os

from workers import SubmitWorker
from image_handler import encode_image_to_base64, decode_base64_to_pixmap
from utils import resource_path, strip_html_tags
from delegates import ComboBoxDelegate  # If needed

class SubmitDialog(QDialog):
    """
    Dialog window to handle the submission of album data to Telegram.
    Emits a signal when the submission is finished.
    """
    ...
    def onSubmissionFinished(self, success, message):
        ...
        if success:
            ...
            self.submission_finished.emit(False, message)


class HelpDialog(QDialog):
    """
    Dialog window to display help information using HTML content.
    """
...
class LogViewerDialog(QDialog):
    """
    Dialog window to display live logs within the application.
    """
...
class ManualAddAlbumDialog(QDialog):
    """
    Dialog window to manually add an album to the album list.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
    ...
    def browse_cover_image(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Select Cover Image", "", "Image Files (*.png *.jpg *.bmp)"
        )
        if file_path:
    ...
    def add_album(self):
        ...
        if not artist or not album or not release_date:
            QMessageBox.warning(
                self, "Input Error",
                "Please fill in all required fields (Artist, Album, Release Date)."
            )
            return
        ...
        if not release_date_formatted:
            QMessageBox.warning(
                self, "Input Error",
                "Invalid date format. Please use DDMMYY, DDMMYYYY, or DD-MM-YYYY."
            )
            return
        ...
        # Call the parent method to add the album to the table
...
class UpdateDialog(QDialog):
    """
    Dialog window to notify the user about available updates.
    """
...
class AboutDialog(QMessageBox):
    ...
    def __init__(self, version, parent=None):
        super().__init__(parent)
        self.setWindowTitle("About SuShe")
